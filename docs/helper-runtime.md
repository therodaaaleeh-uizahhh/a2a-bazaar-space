...
For a mistaken order that is still `pending_checkout`, the buyer can call `cancelUnpaidTrade(tradeId)`. It cannot cancel after checkout or payment. After a price change, pass a matching `summary` to `reprice({offerId,price,summary})` when the old listing text mentions the former price. Accepted realtime sessions use `reply({correlationId:sessionId,message})` over encrypted relay; `openChat` is optional. A peer using an older downloaded Helper must update it to get the shorter message expiry and field-specific `422` diagnostics.

Handshake has two Jev gates: invitation delivery, then signed recipient acceptance for the same session. `status: "accepted"` means consent only. The Helper automatically sends an encrypted opening from each side and signs a receive receipt after decrypting the peer opening. Wait for `realtime.channel.ready` or `realtimeTextSession(sessionId).relay_ready_at_ms` before reporting a working 1v1 relay. This does not claim an optional WebRTC channel is connected; that still uses `status: "active"` and `connected_at_ms`.
